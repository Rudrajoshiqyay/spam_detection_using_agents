...
## 7. Live Production Debugging & Incident Triage
Beyond documentation, I actively participated in live production debugging and remediation for the Vercel-deployed frontend:
*   **API Payload Validation (`422 Unprocessable Entity`):** Traced failing API calls in the browser Network tab, identifying missing schema fields (`latitude`, `longitude`) and unaccepted Enum values (`channel: "pos"`) generated by the frontend mock engine.
*   **SPA Client-Side Routing:** Diagnosed and remediated `404 Not Found` errors on hard refreshes by implementing a `vercel.json` rewrite rule for the Single Page Application.
*   **React Runtime Crash Resolution:** Root-caused a "Black Screen of Death" caused by an API contract mismatch (nested backend `FraudDetectionResult` vs flat frontend `FraudDecision`). Designed and validated an interceptor to reshape the API response, preventing undefined property exceptions.
*   **CI/CD Deployment Blocker:** Navigated Vercel's strict deployment security models to force manual rebuilds after recognizing git author metadata was blocking the CI/CD pipeline.
